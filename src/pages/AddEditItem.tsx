...
import { MENU_CATEGORIES, PREP_TIME_OPTIONS, type MenuItem, type MenuCategory } from "@/types/menu";
  ...
  useEffect(() => {
    if (isEdit) {
      const changed = name !== editItem.name || category !== editItem.category ||
        ...
        prepTime !== editItem.preparationTime || isAvailable !== editItem.isAvailable ||
        imagePreview !== editItem.imageUrl;
      setDirty(changed);
    } else {
      setDirty(name.length > 0 || price.length > 0 || description.length > 0 || !!imagePreview);
    }
  }, [name, category, price, description, prepTime, isAvailable, imagePreview, isEdit, editItem]);
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-5 pb-28 space-y-5">
          ...
          {imagePreview ? (
            <div className="relative mt-2 h-48 rounded-2xl overflow-hidden border-2 border-primary/30 bg-muted">
              ...
              <button
                ...
                className="absolute top-2 right-2 flex h-7 w-7 items-center justify-center rounded-full bg-black/60 active:scale-95 transition-transform"
              >
            ...
            <button
              ...
              className="mt-2 flex h-48 w-full flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-border bg-muted/50 active:border-primary transition-colors"
            >
        ...
        <div>
          ...
          {name.length > 0 && !nameValid && (
            <p className="mt-1 text-[11px] text-destructive">Minimum 3 characters required</p>
          )}
        </div>
        ...
        <div>
          ...
          <select
            ...
            className="mt-2 w-full appearance-none rounded-xl border border-border bg-muted px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
          >
        ...
        <div>
          ...
          <div className="relative mt-2">
            ...
            <input
              ...
              placeholder="0"
              min="1"
              className="w-full rounded-xl border border-border bg-muted pl-8 pr-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="mt-2 w-full resize-none rounded-xl border border-border bg-muted px-4 py-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
          />
          <p className={cn("text-right text-[11px] mt-1", description.length >= 180 ? "text-destructive" : "text-muted-foreground")}>
        ...
        <div className="flex items-center justify-between rounded-xl bg-card border border-border p-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{isAvailable ? "Item is visible to customers" : "Item is hidden"}</p>
          </div>
          <button
      ...
      <div className="fixed bottom-0 left-0 right-0 z-20 border-t border-border bg-card px-4 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
        <div className="mx-auto max-w-md">
          ...
          >
            {saving ? (
              <div className="mx-auto h-5 w-5 rounded-full border-2 border-current/30 border-t-current animate-spin" />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-6">
          ...
          <div className="relative w-full max-w-sm rounded-2xl bg-card p-6">
            ...
            <div className="mt-6 flex gap-3">
              <button onClick={() => setShowDiscard(false)} className="flex-1 rounded-xl border border-border py-3 text-sm font-semibold text-foreground active:scale-95 transition-all">
                Keep Editing
              </button>
              <button onClick={() => navigate("/menu")} className="flex-1 rounded-xl bg-destructive py-3 text-sm font-semibold text-destructive-foreground active:scale-95 transition-all">
                Discard
              </button>
            </div>
          </div>
